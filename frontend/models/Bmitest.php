...
use Yii;
use yii\helpers\Url;
use frontend\models\Gradebmi;
use frontend\models\Gradewaistline;
   ...
   */
    ...
    public function attributeLabels()
    {
        return [
            ...
            'gradebmiName'=>'ระดับดัชนีมวลกาย',
            'waistline'=>'รอบเอว',
            'gradewaistline'=>'ระดับรอบเอว',
            'gradewaistlineName'=>'แปรผลรอบเอว',
            'bpsys'=>'ความดันโลหิตช่วงบน',
            'bpdi'=>'ความดันโลหิตช่วงล่าง',
            'gradebpsys'=>'แปรผลความดันโลหิตช่วงบน',
            'gradebpsysName'=>'แปรผลความดันโลหิตช่วงบน',
            'gradebpdi'=>'แปรผลความดันโลหิตช่วงล่าง',
            'gradebpdiName'=>'แปรผลความดันโลหิตช่วงล่าง',
...
if($sex== 1 and $age >=60 and $run2400 <11.5){
  ...
  $this->graderun = '2' ;
}else if ($sex== 1 and $age >=60 and$run2400 >=20.01){
  $this->graderun = '1' ;
...
//อัพโหลดภาพแบบ ajax//
    ...
    public function getGradebmis(){
        return @$this->hasOne(Gradebmi::className(),['BMI_ID'=>'gradebmi']);
    }
    public function getGradebmiName(){
        return @$this->gradebmis->BMI_NAME;
    }
    
    public function getGradebpsyss(){
        return @$this->hasOne(Gradebpsys::className(),['BPSYS_ID'=>'gradebpsys']);
    }
    public function getGradebpsysName(){
        return @$this->gradebpsyss->BPSYS_NAME;
    }
    
    public function getGradebpdis(){
        return @$this->hasOne(Gradebpdi::className(),['BPDI_ID'=>'gradebpdi']);
    }
    public function getGradebpdiName(){
        return @$this->gradebpdis->BPDI_NAME;
    }
    
    
    public function getGradewasitlines(){
        return @$this->hasOne(Gradewaistline::className(),['WAISTLINE_ID'=>'gradewaistline']);
    }
    public function getGradewaistlineName(){
        return @$this->gradewaistline->WAISTLINE_NAME;
    }
